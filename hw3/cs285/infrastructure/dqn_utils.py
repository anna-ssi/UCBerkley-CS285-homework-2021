...
def get_env_kwargs(env_name):
    if env_name in ['MsPacman-v4', 'PongNoFrameskip-v4']:
        kwargs = {
...
def atari_exploration_schedule(num_timesteps):
    ...
    return LinearSchedule(1000, final_p=0.001)
...
def atari_optimizer(num_timesteps):
    lr_schedule = PiecewiseSchedule(
        [
            (0, 1e-2),
            (num_timesteps / 40, 1e-2),
            (num_timesteps / 8, 5e-3),
        ],
        outside_value=5e-2,
    )
...
class MemoryOptimizedReplayBuffer(object):
    ...
    def _encode_observation(self, idx):
        ...
        # starts from where the episode ended last
