...
class DQNAgent(object):
    ...
    def step_env(self):
        ...
        perform_random_action = True if eps < np.random.random(
        ) or self.t < self.learning_starts else False
        if perform_random_action:
            action = self.env.action_space.sample()
        else:
            lastest_obs = self.replay_buffer.encode_recent_observation()
            action = self.actor.get_action(lastest_obs)
