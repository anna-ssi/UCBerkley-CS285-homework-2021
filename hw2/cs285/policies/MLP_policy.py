...
class MLPPolicy(BasePolicy, nn.Module, metaclass=abc.ABCMeta):
    ...
    def get_action(self, obs: np.ndarray) -> np.ndarray:
        # None returns the whole numpy array
    ...
    def forward(self, observation: torch.FloatTensor):
        if self.discrete:
            ...
            batch_mean = self.mean_net(observation) #TODO: look into this code
            scale_tril = torch.diag(torch.exp(self.logstd))
            # print(scale_tril, batch_mean.shape, scale_tril.shape)
...
class MLPPolicyPG(MLPPolicy):
    ...
    def update(self, observations, actions, advantages, q_values=None):
        ...
        pred_probs = dist.log_prob(actions)

        loss = -torch.sum(pred_probs * advantages)
        ...
        if self.nn_baseline:
            ...
            q_values = normalize(q_values, np.mean(
                q_values), np.mean(q_values))
            q_values = ptu.from_numpy(q_values).requires_grad_()

            loss = self.baseline_loss(pred_values, q_values)
