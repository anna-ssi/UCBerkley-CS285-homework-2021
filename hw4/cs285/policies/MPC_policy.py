...
class MPCPolicy(BasePolicy):
    ...
    def sample_action_sequences(self, num_sequences, horizon, obs=None):
        if self.sample_strategy == 'random' \
            ...
            elit_mean = 0
            elit_std = 1
            for i in range(1, self.cem_iterations):
                ...
                if i == 0:
                    ...
                    action_sequences = np.random.normal(
                        elit_mean, elit_std, size=(num_sequences, horizon, self.ac_dim))
                    action_sequences = np.clip(action_sequences,
                            self.low, self.high)

                # - Get the top `self.cem_num_elites` elites
                #     (Hint: what existing function can we use to compute rewards for
                #      our candidate sequences in order to rank them?)
                sum_rewards = self.evaluate_candidate_sequences(
                    action_sequences, obs)
                elite_idxs = np.argsort(sum_rewards)[-self.cem_num_elites:]
                elites = action_sequences[elite_idxs, :, :]

                # - Update the elite mean and variance
                if i == 0:
                    elit_mean = np.mean(elites, axis=0)
                    elit_std = np.std(elites, axis=0)
                else:
                    elit_mean = self.cem_alpha * \
                        np.mean(elites, axis=0) + (1 - self.cem_alpha) * elit_mean
                    elit_std = self.cem_alpha * \
                        np.std(elites, axis=0) + (1 - self.cem_alpha) * elit_std
            ...
            cem_action = np.random.normal(
                elit_mean, elit_std, size=(horizon, self.ac_dim))
    ...
    def evaluate_candidate_sequences(self, candidate_action_sequences, obs):
        ...
        predicted_sum_of_rewards = []
        for model in self.dyn_models:
            model_rewards = self.calculate_sum_of_rewards(
                obs, candidate_action_sequences, model)
            predicted_sum_of_rewards.append(model_rewards)

        return np.mean(predicted_sum_of_rewards, axis=0)
