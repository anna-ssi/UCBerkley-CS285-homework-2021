...
class AWACAgent(DQNAgent):
    ...
    def get_qvals(self, critic, obs, action):
        ...
        q_value = critic.q_net(obs)[:, action]
        return q_value
    ...
    def estimate_advantage(self, ob_no, ac_na, re_n, next_ob_no, terminal_n, n_actions=10):
        ...
        if self.agent_params['discrete']:
            for i in range(self.agent_params['ac_dim']):
                q_action_value = self.get_qvals(
                    self.exploitation_critic, ob_no, i)
                vals.append(torch.mean(q_action_value))

        else:
            pass

        # TODO look at this part!!!!
        vals = torch.Tensor(vals).to(ptu.device)
        v_pi = dist.probs @ vals
    ...
    def train(self, ob_no, ac_na, re_n, next_ob_no, terminal_n):
        ...
        if (self.t > self.learning_starts
            ...
            expl_bonus = normalize(expl_bonus,
                                   expl_bonus.mean(),
                                   expl_bonus.std())
            ...
            self.awac_actor.update(ob_no, ac_na, advantage)
